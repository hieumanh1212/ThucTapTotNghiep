                ...
                <div class="col-xl-6 col-lg-8">
                    <div class="tp-login-wrapper">
                        ...
                        <div class="tp-login-mail mb-40">
                            <p>Điền các thông tin để đặt lại mật khẩu cho <span style="font-weight: bold; font-size: 15px;">{{ $email }}</span></p>
                        </div>
                            ...
                            <div class="tp-login-input-wrapper">
                                <div class="tp-login-input-box">
                                    <div class="tp-login-input">
                                        <input id="tp_password" type="password" name="password" placeholder="Tối thiểu 6 kí tự">
                                    </div>
                                    ...
                                    <div class="tp-login-input-title">
                                        <label for="tp_password">Mật khẩu mới</label>
                                    </div>
                                </div>
                                <div class="tp-login-input-box">
                                    <div class="tp-login-input">
                                        <input id="tp_repeat_password" type="password" name="repeat_password" placeholder="Tối thiểu 6 kí tự">
                                    </div>
                                    <div class="tp-login-input-title">
                                        <label for="tp_repeat_password"">Nhập lại mật khẩu</label>
                                    </div>
                                </div>
                            </div>
                            <div id="reset_alert" class="alert" style="display: none;">
                                    </div>
                                    <div class=" tp-login-bottom">
                                        <button id="reset_button" type="submit" class="tp-login-btn w-100">Đổi mật khẩu</button>
                                    </div>
                        </form>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        var resetAlert = document.getElementById('reset_alert');

        $("#reset_form").submit(function(e) {
            e.preventDefault();
            $("#reset_button").html('Đang xử lý...');
            $.ajax({
                url: "{{ route('postReset') }}",
                method: "POST",
                ...
                success: function(response) {
                    ...
                    if (response.status == 200) {
                        $("#reset_button").html('Đổi mật khẩu');
                        if (resetAlert.classList.contains('alert-danger')) {
                            resetAlert.classList.remove('alert-danger');
                        }
                        resetAlert.classList.add('alert-success');
                        resetAlert.style.display = 'block';
                        $("#reset_alert").html(response.messages);
                        window.location.href = "{{ route('loginPage') }}";
                    } else {
                        $("#reset_button").html('Đặt lại mật khẩu');
                        if (resetAlert.classList.contains('alert-success')) {
                ...
                error: function(response) {
                    ...
                    resetAlert.classList.add('alert-danger');
                    resetAlert.style.display = 'block';
                    $("#reset_alert").html("Đã có lỗi xảy ra, vui lòng thử lại sau");
                }
            });
        });
    });
