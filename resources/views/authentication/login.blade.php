                ...
                <div class="col-xl-6 col-lg-8">
                    <div class="tp-login-wrapper">
                        <div class="tp-login-top text-center mb-30">
                            <h3 class="tp-login-title">Đăng nhập vào Shofy.</h3>
                            <p>Không có tài khoản? <span><a href="{{ route('registerPage') }}">Đăng kí miễn phí ngay</a></span></p>
                        </div>
                        <div class="tp-login-option">
                            ...
                            <div class="tp-login-mail text-center mb-40">
                                <p>hoặc đăng nhập bằng <a href="#">Email</a></p>
                            </div>
                            <form method="POST" action="#" id="login_form">
                                ...
                                <div class="tp-login-suggetions d-sm-flex align-items-center justify-content-between mb-20">
                                    ...
                                    <div class="tp-login-forgot">
                                        <a href="{{ route('forgotPage') }}">Quên mật khẩu?</a>
                                    </div>
                                </div>
                                <div id="login-alert" class="alert alert-danger" style="display: none;"></div>
                                <div class="tp-login-bottom">
                                    <button id="login_button" type="submit" class="tp-login-btn w-100">Đăng nhập</button>
                                </div>
                            </form>
                        </div>
                    </div>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 200) {
                        if (response.accountType == "Khách hàng") {
                            window.location.href = "{{ route('homepage') }}";
                        } else if (response.accountType == "Nhân viên kho") {
                            window.location.href = "{{ route('NhanVien.NhanVienKho.index') }}";
                        } else if (response.accountType == "Nhân viên bán hàng") {
                            window.location.href = "{{ route('NhanVien.NhanVienBanHang.index') }}";
                        } else if (response.accountType == "Admin") {
                            window.location.href = "{{ route('Admin.Index') }}";
                        }
                    } else if (response.status == 401 || response.status == 404 || response.status == 400 || response.status == 403) {
                        $("#login_button").html('Đăng nhập');
                        loginAlert.style.display = 'block';
                        $("#login-alert").html(response.messages);
                    }
                },
