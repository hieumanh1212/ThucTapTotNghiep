    ...
    <div class="table-responsive">
        <table class="table table-striped ">
            ...
            <tbody>
                ...
                @foreach ($hoadondanggiao as $doncho)
                <tr style="background-color: white; transition: background-color 0.3s ease; cursor: pointer;" onmouseover="this.style.backgroundColor='lightgray'" onmouseout="this.style.backgroundColor='white'" onclick="showModal('{{ $doncho->ma_hoa_don_ban }}', 'Đang giao',
                ...
                '{{ $doncho->so_dien_thoai_nguoi_nhan }}', '{{ $doncho->dia_chi_giao_hang }}')">
                    ...
                    @if($doncho->trang_thai_thanh_toan == 1)
                    <td>Đã thanh toán</td>
                    @else
                    ...
                    <td>
                        <button class="btn btn-success" onclick="hoanThanh('{{ $doncho->ma_hoa_don_ban }}', event)">Hoàn thành</button>
                    </td>
                </tr>
                @endforeach
            </tbody>
...
<script>
    //Hoàn thành đơn hàng
    async function hoanThanh(maHoaDon) {
        event.stopPropagation();

        try {
            const response = await $.ajax({
                type: 'GET',
                url: '{{ route("hoan-thanh-don") }}',
                data: {
                    maHoaDon: maHoaDon,
                },
                dataType: 'json',
