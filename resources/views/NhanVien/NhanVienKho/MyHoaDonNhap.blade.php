...
@section('Title', 'Hóa đơn nhập của bạn')
...
<div style="padding: 20px">
    ...
    <form action="" class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search" style="float: right; display: flex;">
        <div class="input-group">
            <input type="text" name="Key_NVKHoaDonNhap" value="{{ request()->Key_NVKHoaDonNhap }}" class="form-control bg-light border-0 small" placeholder="Tìm kiếm..."
...
<div class="card-body">
    <div class="table-responsive">
                ...
                @foreach ($My_HDN as $item)
                    <tr>
                        ...
                        <td class="text-center">
                            <a href="{{ route('NhanVien.NhanVienKho.show', ['mahoadonnhap' => $item->ma_hoa_don_nhap]) }}" class="btn btn-success">
        ...
        <div>
            {{ $My_HDN->appends(request()->all())->links() }} {{-- dữ nguyên tham số trên URL --}}
        </div>
    </div>
</div>
@endsection
@section('js')
    <script>
        var url = "{{ route('NhanVien.NhanVienKho.index') }}";
    </script>
    @if (session()->has('TaoHDN_thanhcong'))
        <script>Swal.fire('Tạo thành công', '{{ session()->get('TaoHDN_thanhcong') }}' ,'success');</script>
    @endif 
@endsection